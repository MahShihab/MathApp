...
class OceanActivityForNumbersFromZeroToNineWidget extends StatefulWidget {
  @override
  _OceanActivityForNumbersFromZeroToNineWidgetState createState() =>
      _OceanActivityForNumbersFromZeroToNineWidgetState();
}

class _OceanActivityForNumbersFromZeroToNineWidgetState
    extends State<OceanActivityForNumbersFromZeroToNineWidget> {
  ...
  Scaffold buildPageWithStack(int index) {
    if(index==1 && justOneTimeForAudio_whatIsTheNymberOfFishs) {
      ...
      audioPlayer.play('OceanActivity/whatIsTheNymberOfFishs.mp3');
    }
  ...
  Container lastPageInPageView() {
    audioPlayer.play('OceanActivity/thanksForCountFishs.mp3');
    Future.delayed(Duration(seconds: 11)).then((_) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => ModuleforKideWidget(),
        ),
      );
    });
